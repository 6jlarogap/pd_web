Установка у заказчика ПК (виртуальной машины) с копией системы
    ...
    * Участвуют:
        ...
        - посредник, пусть это будет ПК suprune20, см. dealer/*:
            * считывает базу данных всех заказчиков сервера, 
            * разворачивает базу данных у себя
            * убирает из базы данные других организаций

        !!! NB: это делается командой ./manage.py one_ugh_leave ...
            Поскольку это не используется, то one_ugh_leave.py
            (который, после того как его довели до современного способа
             вызова management commands и до python3, 
             еще не испытывался!!!
            ),
            отправлен в contrib/unused-management-commands

            * формирует текст, содержащий только медийные файлы заказчика
            * отправляет архив базы на сервер, в каталог заказчика, а также
              тот текст, содержащий лишь медию заказчика
            * дает команду серверу: в chrooted home folder пользователя
              считать rsync'ом только медиа пользователя, согласно тому тексту
        - виртуальная машина на ПК заказчика:
            ...
            * обновляет каталог проекта, включая миграцию:
                ...
                по структуре данных
            * забирает медийные файлы со своего chrooted home folder 
              на сервере, посредством rsync
    ...
    * Сервер. Положить сценарий del_empty_dirs.py, например в /usr/local/bin.
        rsync, когда копирует лишь файлы из --include-from= списка,
        кроме этих файлов, записывает все пустые каталоги, не удалось
        заставить rsync избежать этого.
        
        Сценарий del_empty_dirs.py ждет на stdin путь к каталогу,
