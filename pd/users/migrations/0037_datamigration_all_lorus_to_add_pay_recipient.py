...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Перевести всех имеющихся лору в реестр организации settings.ORG_AD_PAY_RECIPIENT,
        # чтоб потом можно было использовать эти ЛОРУ в публичном каталоге,
        # "держателем" которого и есть settings.ORG_AD_PAY_RECIPIENT
        #
        print "*** Adding all loru's to settings.ORG_AD_PAY_RECIPIENT registry"
        Org = orm['users.Org']
        print "*** Looking for settings.ORG_AD_PAY_RECIPIENT"
        add_pay_recipient = Org.objects.get(inn=settings.ORG_AD_PAY_RECIPIENT['inn'])
        print "***  found"
        print "*** Making sure it is OMS"
        if add_pay_recipient.type != 'ugh':
            print "***  it is not oms, making it oms"
            add_pay_recipient.type = 'ugh'
            add_pay_recipient.save()
        cnt_added = cnt_existing = 0
        ProfileLORU = orm['users.ProfileLORU']
        for loru in Org.objects.filter(type='loru'):
            rec, created = ProfileLORU.objects.get_or_create(ugh=add_pay_recipient, loru=loru)
            if created:
                cnt_added += 1
            else:
                cnt_existing += 1
        print "*** LORUs, total: %d, added to settings.ORG_AD_PAY_RECIPIENT registry: %d, already there: %d" % \
              (cnt_added+cnt_existing, cnt_added, cnt_existing, )
