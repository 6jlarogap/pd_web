...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # У ряда пользователей в профиле нет ФИО, а в модели User имеется фамилия в last_name,
        # и имя отчество в first_name. Всё надо перенести в профиль
        #
        print "*** Filling Profile empty FIOs by User.last_name, User.first_name"
        Profile=orm['users.Profile']
        q = (Q(user_last_name__regex=r'^\s*$') | Q(user_last_name__isnull=True)) & \
            Q(user__last_name__regex=r'\S+')
        count = 0
        for p in Profile.objects.filter(q):
            p.user_last_name = p.user.last_name.strip()
            io = (p.user.first_name or '').split(None, 1)
            p.user_first_name = (len(io) > 0 and io[0] or '').strip()
            p.user_middle_name = (len(io) > 1 and io[1] or '').strip()
            p.save()
            count +=1
        print "*** %d profiles filled" % count
