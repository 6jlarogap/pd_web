...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Org:  dt_created : первое упоминание организации в журнале,
        #       dt_modified : последнее упоминание организации в журнале,
        #
        print "*** Org: dt_created, dt_modified from logs"
        Org = orm['users.Org']
        ContentType = orm['contenttypes.ContentType']
        try:
            ct_org = ContentType.objects.get(app_label='users',model='org').id
            count_all = count_fake = 0
            Log = orm['logs.Log']
            for o in Org.objects.all():
                count_all += 1
                org_logs = Log.objects.filter(ct=ct_org, obj_id=o.pk)
                try:
                    dt_created = org_logs.order_by('id')[0].dt
                    dt_modified = org_logs.order_by('-id')[0].dt
                    Org.objects.filter(pk=o.pk).update(dt_created=dt_created, dt_modified=dt_modified, )
                except IndexError:
                    count_fake += 1
            print "***     %s*2 dt's all, %s*2 dt's left now(): no recs in logs" % (count_all, count_fake, )
        except (ContentType.DoesNotExist, ContentType.MultipleObjectsReturned):
            print "***     %s*2 dt's all left now(): Org obj_id for logs" % Org.objects.all().count()
