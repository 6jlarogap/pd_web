...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "*** Add admin & registrator roles, assign then to all oms users"
        print "*** Assign all OMS cemeteries to all oms users"
        Role = orm['users.Role']
        role_admin, created_ = Role.objects.get_or_create(
            name='admin',
            defaults = dict(title=u'Администратор'))
        role_registrator, created_ = Role.objects.get_or_create(
            name='registrator',
            defaults=dict(title=u'Регистратор или смотритель'))

        Profile = orm['users.Profile']
        Cemetery = orm['burials.Cemetery']
        for profile in Profile.objects.filter(org__type='ugh', user__is_active=True):
            profile.role.add(role_admin)
            profile.role.add(role_registrator)
            for cemetery in Cemetery.objects.filter(ugh=profile.org):
    ...
    models = {
        ...
        'users.customerprofile': {
            ...
            'disposable_token': ('django.db.models.fields.CharField', [], {'max_length': '255', 'unique': 'True', 'null': 'True'}),
