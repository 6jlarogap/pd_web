...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Подготовить к уникальным сочетаниям ugh, loru в ProfileLORU
        print "*** Make unique combinations ugh, loru in ProfileLORU"
        ProfileLORU = orm['users.ProfileLORU']
        for pl in ProfileLORU.objects.all():
            ProfileLORU.objects.filter(
                ugh=pl.ugh,
                loru=pl.loru,
            ).exclude(pk=pl.pk).delete()
