...
class Wallet(models.Model):
    org = models.ForeignKey(Org, verbose_name=_("Организация"))
    currency = models.ForeignKey(Currency, verbose_name=_("Валюта"))
...
class Rate(models.Model):
    ...
    wallet = models.ForeignKey(Wallet, verbose_name=_("Кошелек"))
...
class Payment(models.Model):
    ...
    wallet_from = models.ForeignKey(Wallet, verbose_name=_("Кошелек откуда"), related_name='payment_from', null=True)
    wallet_to = models.ForeignKey(Wallet, verbose_name=_("Кошелек куда"), related_name='payment_to', null=True)
    ...
    ct = models.ForeignKey('contenttypes.ContentType', verbose_name=_("Вид платежа"))
...
class Ad(models.Model):
    payment = models.OneToOneField(Payment)
    ugh = models.ForeignKey(Org, limit_choices_to={'type': Org.PROFILE_UGH}, verbose_name=_("ОМС"))
    product = models.ForeignKey('orders.Product', verbose_name=_("Продукт"))
    action = models.CharField(_("Действие"), max_length=255, choices=Rate.RATE_ACTIONS)
    # снятие с показа - тариф, в котором null=True:
    rate = models.ForeignKey(Rate, verbose_name=_("Тариф"), null=True)

class Commission(models.Model):
    payment = models.OneToOneField(Payment)
    share = models.FloatField(_("Процент"))
    source_ct = models.ForeignKey('contenttypes.ContentType', verbose_name=_("Вид платежа, за что комиссия"))
