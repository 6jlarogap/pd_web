...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Order: dt_created из поля Order.created,
        #        dt_modified -- из журнала
        # Кроме того, формируем поле dt_temp из dt_created (datetime)
        #
        print "*** Order: dt_created from Order.created, dt_modified from logs"
        Order = orm['orders.Order']
        ContentType = orm['contenttypes.ContentType']
        try:
            ct_org = ContentType.objects.get(app_label='orders',model='order').id
        ...
        for o in Order.objects.all():
            ...
            if ct_org:
                try:
                    dt_modified = Log.objects.filter(ct=ct_org, obj_id=o.pk).order_by('-id')[0].dt
                except IndexError:
                    pass
            if not dt_created:
            ...
            Order.objects.filter(pk=o.pk).update(
                ...
                dt_temp=datetime.date(year=dt_created.year, month=dt_created.month, day=dt_created.day)
           )
        print "***     %s*2 dt's all, %s dt_created's faked: empty Org.changed,\n" \
    ...
    models = {
        ...
        'orders.order': {
            ...
            'dt_created': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'dt_modified': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
            'dt_temp': ('django.db.models.fields.DateField', [], {}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
