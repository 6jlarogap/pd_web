...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Удалить единицы измерения, которые пока не нужны (килограммы и метры кубические за доставку)
        measures_pks = (3, 4, )
        Measure = orm['orders.Measure']
        OrgServicePrice = orm['orders.OrgServicePrice']
        for measure_pk in measures_pks:
            OrgServicePrice.objects.filter(measure__pk = measure_pk).delete()
            Measure.objects.filter(pk=measure_pk).delete()
