...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Заполнить добавленные предыдущей мграцией добавились поля в Product:
        # * is_public_catalog
        # * price_wholesale
        
        def get_catalog_org_pk():
            if settings.ORG_AD_PAY_RECIPIENT_PK is None:
                try:
                    return orm['users.Org'].objects.filter(inn=settings.ORG_AD_PAY_RECIPIENT['inn'])[0].pk
                except IndexError:
                    raise Exception(u'Falied to get Catalog OMS pk!')
            else:
                return settings.ORG_AD_PAY_RECIPIENT_PK

        def is_public_catalog(product, catalog_org_pk):
            ProductStatus = orm['orders.ProductStatus']
            return ProductStatus.objects.filter(
                product=product,
                status__in=('publish', 'update',),
                ugh__pk=catalog_org_pk,
            ).exists()

        print '*** Filling Product.price_wholesale by Product.price'
        print '*** Filling Product.is_public_catalog'
        catalog_org_pk = get_catalog_org_pk()
        count_all = count_catalog = 0
        for p in orm['orders.Product'].objects.all():
            count_all += 1 
            p.price_wholesale = p.price
            p.is_public_catalog = is_public_catalog(p, catalog_org_pk)
            if p.is_public_catalog:
                count_catalog += 1
            p.save()
        print "*** %d products total, %d of them in public catalog" % (count_all, count_catalog,)
    ...
    models = {
        ...
        'orders.product': {
            ...
            'dt_modified': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'is_component': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            ...
            'price_wholesale': ('django.db.models.fields.DecimalField', [], {'default': "'0.00'", 'max_digits': '20', 'decimal_places': '2'}),
