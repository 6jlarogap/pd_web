...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "*** Filling Org-LORU.currencies with those of their Products"

        Location = orm['geo.Location']
        Country = orm['geo.Country']
        Currency = orm['billing.Currency']

        DOMAINS_ = dict(
            ru=dict(name=u'Россия', obj_country=None, obj_currency=Currency.objects.get(code=u'RUR')),
        ...
        for o in Org.objects.only("pk", "off_address", "currency").filter(type='loru'):
            ...
            if products_currency is None:
                # No products found
                if country is None:
                    if not o.off_address:
                        o.off_address = Location.objects.create()
                        o.save()
                    o.off_address.country = DOMAINS_['ru']['obj_country']
                    o.off_address.save()
                    cnt1 += 1
                else:
                    for domain in DOMAINS_:
                        if o.off_address.country == DOMAINS_[domain]['obj_country']:
                            o.currency = DOMAINS_[domain]['obj_currency']
                            o.save()
                            cnt2 += 1
                            break
            else:
                # Есть продукты, а у них валюта
                if not same_currency:
                    print "!!! Org.pk=%s, products with different currency found!" % o.pk
                    continue
                if not o.currency or products_currency != o.currency:
                    o.currency = products_currency
                    o.save()
                    cnt3 += 1

        print "***  %d lorus with no products/no country are filled with country: Russia" % cnt1
        print "***  %d lorus with no products assigned the currency of their countries" % cnt2
        print "***  %d lorus with products assigned the currency of their products" % cnt3
    ...
    models = {
        ...
        'orders.iorder': {
            ...
            'customer': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'iorder_customers'", 'to': "orm['users.Org']"}),
            'dt_created': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'dt_modified': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
