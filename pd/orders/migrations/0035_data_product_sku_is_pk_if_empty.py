...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Если у продукта не задан артикул (sku), то он устанавливается
        # в первичный ключ продукта
        #
        print "*** Filling product.sku by product.pk if empty product.sku"
        count = 0
        Product = orm['orders.Product']
        for p in Product.objects.filter(sku__regex=r'^\s*$'):
            p.sku = str(p.pk)
            p.save()
            count += 1
        print "*** %d product.sku's filled" % count
