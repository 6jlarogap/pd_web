...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Назначить последние фотки результатов заказа титульными
        Order = orm['orders.Order']
        ResultFile = orm['orders.ResultFile']
        count = 0
        print '*** Assign is_title flag to last order result photos'
        for order in Order.objects.filter(resultfile__isnull=False):
            try:
                last_photo_result = ResultFile.objects.filter(order=order, type='image'). \
                                    order_by('-date_of_creation')[0]
                ResultFile.objects.filter(pk=last_photo_result.pk).update(is_title=True)
                count += 1
            except IndexError:
                pass
        print '*** %d is_title flags assigned' % count
