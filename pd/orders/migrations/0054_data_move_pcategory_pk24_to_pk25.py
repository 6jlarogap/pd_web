...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # - перенести товары категории "Накладки ритуальные" в "Фурнитура для гробов"
        # - удалить категорию "Накладки ритуальные"
        
        print '*** moving Product with category "Nakladki ritualnyie" to "Furnitura dlia grobov"'
        Product = orm['orders.Product']
        ProductCategory = orm['orders.ProductCategory']
        try:
            pc_old = ProductCategory.objects.get(pk=24)
        except ProductCategory.DoesNotExist:
            print '*** product category "Nakladki ritualnyie" seems to be already removed'
            return

        pc_new = ProductCategory.objects.get(pk=25)
        updated = Product.objects.filter(productcategory=pc_old).update(productcategory=pc_new)
        pc_old.delete()
        print '%s products moved, category "Nakladki ritualnyie" removed' % updated
