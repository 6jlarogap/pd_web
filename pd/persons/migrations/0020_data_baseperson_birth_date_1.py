...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Скопировать все даты рождения у DeadPerson в BasePerson.birth_date_temp
        #
        # Используем для ускорения, чтоб не перебирать все записи, update всей таблицы.
        # Кроме того, вынуждены применить raw запрос, т.к., например,
        # BasePerson.objects.all().update(birth_date_temp=F(deadperson__birth_date))
        # не работает: joined field references are not permitted in this query
        #
        # NB: sql: coalesce(f1, f2) = f1 if f1 is not null else f2
        #
        raw_query = '''
            update persons_baseperson set {0} = 
            coalesce(
                (
                    select persons_deadperson.{1} from persons_deadperson
                    where baseperson_ptr_id = persons_baseperson.id
                ),
                {0}
            )
            ;
        ...
        for args in (
                ('birth_date_temp', 'birth_date', ),
                ('birth_date_temp_no_day', 'birth_date_no_day', ),
                ('birth_date_temp_no_month', 'birth_date_no_month', ),
            ):
            cursor.execute(raw_query.format(*args))
    ...
    models = {
        ...
        'persons.baseperson': {
            ...
            'birth_date_temp': ('pd.models.UnclearDateModelField', [], {'null': 'True', 'blank': 'True'}),
            u'birth_date_temp_no_day': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            u'birth_date_temp_no_month': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
