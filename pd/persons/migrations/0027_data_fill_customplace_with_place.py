...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Сделать объекты CustomPlace из тех Place, где есть ответственный за место захоронения
        #
        print "*** Making CustomPlaces linked to Places with responsible users"
        Place = orm['burials.Place']
        CustomPlace = orm['persons.CustomPlace']
        places = Place.objects.filter(responsible__user__isnull=False)
        for place in places:
            CustomPlace.objects.create(
                user=place.responsible.user,
                place=place,
            )
        print "*** %s CustomPlaces created" % places.count()
