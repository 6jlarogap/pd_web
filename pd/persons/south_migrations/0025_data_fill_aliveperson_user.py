...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Заполнить AlivePerson.user данными пользователей- ответственных за место
        # с телефоном
        #
        print "*** AlivePerson: fill 'user' field by CustomerProfile.user, by means "
        print "*** of AlivePerson:login_phone -- CustomerProfile.login_phone link"
        cnt_all = cnt_linked = cnt_not_linked = 0
        AlivePerson = orm['persons.AlivePerson']
        CustomerProfile = orm['users.CustomerProfile']
        User = orm['auth.User']
        for person in AlivePerson.objects.filter(login_phone__isnull=False):
            cnt_all += 1
            try:
                customerprofile = CustomerProfile.objects.get(login_phone=person.login_phone)
                cnt_linked += 1
            except CustomerProfile.DoesNotExist:
                cnt_not_linked += 1
                # Проверим, не привязан ли этот alive person c login_phone к месту (местам).
                # Если не привязан, ничего страшного, создадим новый CustomerProfile.
                # Когда-то в ОМС только записывали login_phones,
                # а CustomerProfile + User появлялись после того как пользователь
                # попросил заменить ему пароль по смс
                #
                chars = string.ascii_uppercase + string.ascii_lowercase + string.digits
                user = User.objects.create(
                    username=person.login_phone,
                    password=make_password(''.join(random.choice(chars) for x in range(5))),
                    is_active=True,
                    email=None,
                )
                customerprofile = CustomerProfile.objects.create(
                    ...
                    user_middle_name=person.middle_name,
                    login_phone=person.login_phone,
                )
            AlivePerson.objects.filter(pk=person.pk).update(user=customerprofile.user)

        print "*** Total %s alive persons with login_phone" % cnt_all
        print "*** %s of the total updated with user field" % cnt_linked
        print "*** %s of the total got a customer login & random password" % cnt_not_linked
