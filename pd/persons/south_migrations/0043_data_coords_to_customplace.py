...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Была ошибка, при создании CustomPlace из Place не заносились
        # координаты места. Исправляю

        print "*** Putting coordinates from Place to CustomPlace, if the latter has no coordinates"
        Place = orm['burials.Place']
        CustomPlace = orm['persons.CustomPlace']
        Location = orm['geo.LOcation']
        # Есть координаты у Place, нет координат у CustomPlace
        customplace_qs = CustomPlace.objects.filter(
            place__lat__isnull=False,
            place__lng__isnull=False,
            address__gps_x__isnull=True,
            address__gps_y__isnull=True,
            ).distinct()
        count = customplace_qs.count()
        for customplace in customplace_qs:
            ...
            if not customplace.address:
                ...
                customplace.save()
        print "*** %s CustomPlaces filled with address" % count
