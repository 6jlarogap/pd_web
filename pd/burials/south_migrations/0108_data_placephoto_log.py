...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # До этой миграции выгрузка фото мест журналировалась только текстовым сообщением
        # типа:
        #       Прикреплено фото: <url>
        # 1.    Эту текстовую запись меняем в журнале на <url> и дописываем операцию
        #       LogOPeration.PHOTO_TO_PLACE_MOBILE (14)
        #
        # 2.    Был период, когда выгрузка фото журналировалась в contenttype =
        #       PlacePhoto_ct. Надо подправить соответствующие записи в журнале:
        #           contenttype = Place content_type
        #           obj_id = placephoto.place.pk
        # 4.    Был период, когда выгрузка фото не журналировалась. Правим:
        #       если обнаруживается создатель, фото,то вносим в журнал запись:
        #           contenttype = Place content_type
        #           obj_id = placephoto.place.pk
        #           dt = placephoto.date_of_creation
        # 3.    Был период, когда выгрузка мест не журналировалась. Правим:
        #       если у места есть фото, оттуда берем создателя. Дата создания места --
        #       из свойств самого места
        ...
        place_ct = ContentType.objects.get(app_label__iexact='burials', model__iexact='place')
        placephoto_ct = ContentType.objects.get(
            app_label__iexact='burials',
            model__iexact='placephoto'
        )
        
        print "*** Photo attached to place, log msg to operation"

        print "    - Log recs like 'Prikrepleno photo'"
        count_prikrepleno = count_prikrepleno_error = 0
        for l in Log.objects.filter(
                msg__iregex=u"^Прикреплено фото\\: \\S+",
                ct=place_ct,
            ).iterator():
            m = re.search(u"^Прикреплено фото\\: (\\S+)", l.msg)
            if m:
                count_prikrepleno += 1
                ...
                count_prikrepleno_error += 1
        print "      %s log recs updated, errors: %s" % (
            count_prikrepleno,
        ...
        for l in Log.objects.filter(
                    ct=placephoto_ct,
                    msg__regex=u"^Фото к месту \\'\\S+\\' создано через мобильное приложение"
                 ).iterator():
            count += 1
            try:
                ph = PlacePhoto.objects.get(pk=l.obj_id)
                Log.objects.filter(pk=l.pk).update(
                    ct=place_ct,
                    obj_id=ph.place.pk,
                ...
                count_not_found += 1
                continue
        print "      %s log recs updated, placePhotos from log not found: %s" % (
            count,
            count_not_found,
        )
        ...
        count = 0
        for p in Place.objects.filter(is_invent=True).iterator():
            if not Log.objects.filter(
                    ct=place_ct,
                    obj_id=p.pk,
                ...
                try:
                    ph = PlacePhoto.objects.filter(place=p).order_by('date_of_creation')[0]
                    user = ph.creator
                    if not user:
                        continue
                    logrec = Log.objects.create(
                    ...
                    dt = min(p.dt_created, ph.date_of_creation)
                    Log.objects.filter(pk=logrec.pk).update(
                        dt=dt,
                    )
                    count += 1
    ...
    models = {
        ...
        'burials.burial1': {
            ...
            'place_number': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True'}),
            ...
            'row': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True'}),
            'source_type': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True'}),
        ...
        'logs.log': {
            'Meta': {'ordering': "['-dt']", 'object_name': 'Log'},
            ...
            'dt': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'db_index': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'msg': ('django.db.models.fields.TextField', [], {}),
            'obj_id': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True', 'db_index': 'True'}),
            'operation': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True', 'db_index': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']", 'null': 'True'})
        },
    }
