...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Уникальные имена участков внутри одного кладбища
        
        print "*** Creating unique names for areas"
        Area = orm['burials.Area']
        Cemetery = orm['burials.Cemetery']
        count_all = count_empty = count_doubles = 0
        for c in Cemetery.objects.all():
            fake_num = fake_num_doubles = 1
            for a in Area.objects.filter(cemetery=c):
                count_all += 1
                if a.name.strip():
                    # Непустые проверяем на совпадения. Если такое будет,
                    # то назначаем "старое_имя-номер_по_порядку"
                    a_name = a.name
                    while Area.objects.filter(cemetery=c, name=a_name).exclude(pk=a.pk).exists():
                        a_name = u"%s-%s" % (a.name, fake_num_doubles, )
                        fake_num_doubles += 1
                    if a_name != a.name:
                        # нашлось таки совпадение
                        Area.objects.filter(pk=a.pk).update(name=a_name, )
                        fake_num_doubles += 1
                        count_doubles += 1
                else:
                    # Пустым именам назначаем порядковые номера
                    while Area.objects.filter(cemetery=c, name=str(fake_num)).exists():
                        fake_num += 1
                    # Кстати, ...update(), в отличие от save() не меняет dt_modified. Удобно
                    Area.objects.filter(pk=a.pk).update(name=str(fake_num), )
                    fake_num += 1
                    count_empty += 1
                    
        print ("***  %s areas total, \n"
               "     %s of total had empty names,\n"
               "     %s of total had non-empty names but had doubles"
              ) % \
              (count_all, count_empty, count_doubles)
