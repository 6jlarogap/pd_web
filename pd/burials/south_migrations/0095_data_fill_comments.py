...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "Fill BurialComment by existing comments"
        count = 0
        ContentType = orm['contenttypes.ContentType']
        ct = ContentType.objects.get(app_label__iexact="burials", model__iexact="Burial")
        Log = orm['logs.Log']
        Burial = orm['burials.Burial']
        BurialComment = orm['burials.BurialComment']
        comment_prefix = u'Комментарий: '
        for log in Log.objects.filter(ct=ct, user__isnull=False, msg__istartswith=comment_prefix):
            pure_comment = log.msg[len(comment_prefix):].strip()
            if pure_comment:
                try:
                    burial = Burial.objects.get(pk=log.obj_id)
                    ...
                    pass
        print "%s burial comments created." % count
    ...
    models = {
        ...
        'burials.burialcomment': {
            'Meta': {'object_name': 'BurialComment'},
