...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for burial in orm['burials.Burial'].objects.all():
            if burial.burial_type == Burial.BURIAL_URN:
                Burial.objects.filter(pk=burial.pk).update(burial_container=Burial.CONTAINER_URN)
            else:
                Burial.objects.filter(pk=burial.pk).update(burial_container=Burial.CONTAINER_COFFIN)
