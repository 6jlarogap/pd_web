...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print 'Put place dt_created in the corresponding log record'
        Log = orm['logs.Log']
        Place = orm['burials.Place']
        count = count_error = count_success = 0
        for l in Log.objects.filter(operation=6):
            count += 1
            try:
                p = Place.objects.get(pk=l.obj_id)
                Log.objects.filter(pk=l.pk).update(dt=p.dt_created)
                count_success += 1
            except Place.DoesNotExist:
                count_error += 1
                pass
        print " Total logrecs: %s, successfully edited: %s, error looking for the palce: %s" % (
            count,
            count_success,
            count_error,
        )
    ...
    models = {
        ...
        'billing.currency': {
            'Meta': {'unique_together': "(('code',),)", 'object_name': 'Currency'},'code': ('django.db.models.fields.CharField', [], {'max_length': '10'}),
