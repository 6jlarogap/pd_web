...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for c in orm['burials.Cemetery'].objects.all():
            try:
                c.ugh = c.creator.profile.org.ugh_list.all()[0].ugh
            except:
                try:
                    c.ugh = orm['users.Org'].objects.filter(type=Org.PROFILE_UGH)[0]
                except:
                    pass
            c.save()
    ...
    models = {
        ...
        'burials.burialrequest': {
            ...
            'connected_ugh': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'related_name': "'connected_requests'", 'blank': 'True', 'to': "orm['users.Org']"}),
