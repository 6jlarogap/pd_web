...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Добавилась галка "Документ не обязателен" (Burial.flag_no_applicant_doc_required)
        # При заявителе-фл захоронения необязательно указывать документ.
        # Надо установить эту галку в существующих зх при миграции:
        # По умолчанию галка не установлена (это сделано при при предыдущей миграции,
        # где flag_no_applicant_doc_required default = False)
        # - если зх имеет заявителя-фл и
        # (- если у этого заявителя-фл нет документа или
        #   -если в документе не все заполнено)
        #  устанавливаю галку - в True
        
        Burial = orm['burials.Burial']
        Personid = orm['persons.personid']
        for b in Burial.objects.filter(applicant__isnull=False):
            flag_no_applicant_doc_required = False
            try:
                personid = Personid.objects.get(person=b.applicant)
                if not (personid.id_type and personid.series and personid.number):
                    flag_no_applicant_doc_required = True
            except Personid.DoesNotExist:
                flag_no_applicant_doc_required = True
            if flag_no_applicant_doc_required:
