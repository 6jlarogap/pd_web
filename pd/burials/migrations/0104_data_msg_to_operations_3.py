...
class Migration(DataMigration):
    ...
    depends_on = (
        ("logs", "0008_data_msg_to_operations_2"),
    )
    ...
    def forwards(self, orm):
        ...
        # Заменить в журнале сообщения на операции
        #
        print '*** Change messages "Closed Burial" in Log by their operation counterparts'
        Log = orm['logs.Log']
        ...
        operations = dict(
            full=LogOperation.CLOSED_BURIAL_FULL,
            ugh=LogOperation.CLOSED_BURIAL_UGH,
            archive=LogOperation.CLOSED_BURIAL_ARCHIVE,
        ...
        for b in Burial.objects.filter(status__in=('closed', 'exhumated',)).iterator():
            ...
            elif b.source_type == 'transferred':
                # Импорт: Минские кладбища
                n = Log.objects.filter(
                    ct=burial_ct,
                    obj_id=int(b.pk),
                    msg=u'Импорт',
                    operation__isnull=True,
