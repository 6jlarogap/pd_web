...
class LogOperation(object):
    ...
    Operation = [
        ...
        _('Снятие признака "Занято" для могилы'),                                          #  2
        _('Фотографии места обработаны'),                                                  #  3
        _('Захоронение добавлено по фото'),                                                #  4
        _('Брак фото места'),                                                              #  5
        _('Место создано через мобильное приложение'),                                     #  6
        _('Приглашение в ХРАМ, от ЛОРУ'),                                                  #  7
        _('Фотографирование пользовательского места, ЛОРУ'),                               #  8
        _('Захоронение закрыто и прикреплено к могиле в мобильном приложении'),            #  9
        _('Импорт захоронения'),                                                           # 10
        _('Архивное захоронение закрыто'),                                                 # 11
        _('Электронное захоронение закрыто'),                                              # 12
        _('Ручное захоронение закрыто'),                                                   # 13
...
class Log(models.Model):
    ...
    msg = models.TextField(editable=False, verbose_name=_("Описание"))
    code = models.CharField(max_length=255, default='', editable=False, verbose_name=_("Спец. код"))
    operation = models.PositiveIntegerField(null=True, editable=False, verbose_name=_("Код операции"), db_index=True)
    ...
    def log_object_display(self):
        ...
        if model_name == 'Burial':
            ...
            result = _("Захоронение <a href='%(ref)s'>%(obj_id)s</a>") % dict(
            ...
            try:
                ...
                href = _("не найдена")
            result = _("Могила  № %(grave_number)s, место %(place_url)s") % dict(
            ...
            try:
                ...
                loru_number = ''
            result = _("Заказ <a href='%(ref)s'>%(loru_number)s</a>") % dict(
            ...
            try:
               ...
               result = _("Организация")
        elif model_name == 'User':
            try:
                result = _("Пользователь")
...
def compare_obj(verbose_name, old_val, new_val):
    ...
    if old_val=="None":
        res = _("'%(verbose_name)s': добавлено '%(new_val)s'") % dict(
            verbose_name=verbose_name, new_val=new_val
        )
    elif new_val=="None":
        res = _("'%(verbose_name)s': удалено '%(old_val)s'") % dict(
            verbose_name=verbose_name, old_val=old_val
        )
    elif old_val != new_val:
        res = _("'%(verbose_name)s': '%(old_val)s' -> '%(new_val)s'") % dict(
...
def log_object(request, obj=None, old=None, new=None, reason=None, footer=None, \
    ...
    if old is None:
        msg.append(create_text or _('Создан "%s"') % str(new))
    elif new is None:
        msg.append(delete_text or _('Удален "%s"') % str(old))
    else:
...
def prepare_m2m_log(verbose_name="", old_set = [], new_set=[]):
    ...
    for i in new_set:
        new_arr[i.id] = i
    new_arr_keys = list(new_arr.keys())
    ...
    old_arr_keys = list(old_arr.keys())
...
class DeleteLog(models.Model):
    ...
    parent_id = models.PositiveIntegerField(null=True, editable=False, verbose_name=_("ID родителя"), db_index=True)
