...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print '     - PLACE_PHOTO_REJECT'
        Log.objects.filter(msg=u'Заказано повторное фото места').update(
            msg='',
            operation=LogOperation.PLACE_PHOTO_REJECT,
        )

        print '     - PLACE_PHOTO_PROCESSED'
        Log.objects.filter(msg=u'Фотографии места обработаны').update(
            msg='',
            operation=LogOperation.PLACE_PHOTO_PROCESSED,
        )

        print '     - BURIAL_PHOTO_PROCESSED'
        Log.objects.filter(msg=u'Захоронение внесено при обработке фото места').update(
            msg='',
            operation=LogOperation.BURIAL_PHOTO_PROCESSED,
        )

        print '     - PLACE_CREATED_MOBILE'
        Log.objects.filter(msg__iregex=u"Место \\'\\d+\\' создано через мобильное приложение").update(
            msg='',
